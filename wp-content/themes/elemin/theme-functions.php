	   ...
	   ?>
		<li id="comment-<?php comment_ID() ?>">
			<p class="comment-author">
				...
				<time datetime="<?php comment_date('o-m-d'); ?>T<?php comment_time('H:i'); ?>" class="comment-time"><?php comment_date( apply_filters( 'themify_comment_date', '' ) ); ?> @ <?php comment_time( apply_filters( 'themify_comment_time', '' ) ); ?><?php edit_comment_link( __('Edit', 'themify'),' [',']') ?></time>
			</p>
...
/**
 * Check whether header slider must be displayed or not.
 *
 * @return bool
 */
function themify_theme_show_header_slider() {
	if ( ( is_front_page() || ( !is_front_page() && themify_get( 'setting-header_slider_all_pages' ) ) ) && !is_paged() ) {
		return true;
	}
	return false;
}
...
function themify_theme_layout_before(){
	...
	if ( themify_theme_show_header_slider() ) {
