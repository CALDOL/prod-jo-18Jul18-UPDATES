...
/***************************************************************************/	
	...
	function themify_header_slider($data=array()){
		...
		$slider_ops = array( __( 'Off', 'themify' ) => 'off', __( 'On', 'themify' ) => 'on' );
		/** Slider status */
		$enabled_display = '';
		$display_posts = '';
		$display_images = '';
		...
		if ( 'on' == themify_get( 'setting-header_slider_enabled' ) ) {
		...
		$output .= sprintf('<p class="pushlabel"><label for="%1$s"><input type="checkbox" id="%1$s" name="%1$s" %2$s /> %3$s</label></p>',
			'setting-header_slider_all_pages',
			checked( themify_get( 'setting-header_slider_all_pages' ), 'on', false ),
			__( 'Display header slider on all pages.', 'themify' )
		);

		$output .= '<p><span class="label">' . __('Display', 'themify') . '</span> <input type="radio" class="feature-box-display" name="setting-header_slider_display" value="posts" '.$display_posts.' /> ' . __('Posts', 'themify') . ' <input type="radio" class="feature-box-display" name="setting-header_slider_display" value="images" '.$display_images.' /> ' . __('Images', 'themify') . '</p>
		...
		$output .=	'	<input type="text" name="setting-header_slider_posts_slides" value="' . themify_get( 'setting-header_slider_posts_slides' ) . '" class="width2" /> ' . __('number of posts to be queried', 'themify') . ' 
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						<span class="label">' . __('Content Display', 'themify') . ' </span>
								...
								foreach ( $display_options as $option => $value ) {
									$output .= '<option value="' . $value . '" ' . selected( themify_get( 'setting-header_slider_default_display' ), $value, false ) . '>' . $option . '</option>';
								}
			...
			$output .= '<p class="feature_box_posts" '.$display_posts_display.'>
						...
						foreach ( $title_options as $name => $option ) {
							$output .= '<option value="' . $option . '" ' . selected( themify_get( 'setting-header_slider_hide_title' ), $option, false ) . '>' . $name . '</option>';
						}
		$output .= '
						</select>
					</p>';
		...
		$output .= '</div>
					...
					<p>
						...
						for ( $x = 1; $x <= 7; $x ++ ) {
							$output .= '<option value="' . $x . '"' . selected( themify_check( 'setting-header_slider_visible' ) ? themify_get( 'setting-header_slider_visible' ) : 4, $x, false ) . '>' . $x . '</option>';
						}
	...
	function themify_footer_slider($data=array()){
		...
		$display_posts_display = '';
		$display_images_display = '';
		if ( 'on' == themify_get( 'setting-footer_slider_enabled' ) ) {
			$enabled_checked = "selected='selected'";
		} else {
			$enabled_display = "style='display:none;'";	
		}
		...
		$output .=	'	<input type="text" name="setting-footer_slider_posts_slides" value="' . themify_get( 'setting-footer_slider_posts_slides' ) . '" class="width2" /> ' . __('number of posts to be queried', 'themify') . ' 
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						<span class="label">' . __('Content Display', 'themify') . ' </span>
								...
								foreach($display_options as $option => $value){
									$output .= '<option value="'.$value.'" ' . selected( themify_get( 'setting-footer_slider_default_display' ), $value, false ) . '>'.$option.'</option>';
								}
			...
			$output .= '<p class="feature_box_posts" '.$display_posts_display.'>
						...
						';
						foreach ( $title_options as $name => $option ) {
							$output .= '<option value="' . $option . '" ' . selected( themify_get( 'setting-footer_slider_hide_title' ), $option, false ) . '>' . $name . '</option>';
						}
		$output .= '
						</select>
					</p>';
		...
		$output .= '</div>
					...
					<p>
						...
						for ( $x = 1; $x <= 7; $x ++ ) {
							$output .= '<option value="' . $x . '"' . selected( themify_check( 'setting-footer_slider_visible' ) ? themify_get( 'setting-footer_slider_visible' ) : 4, $x, false ) . '>' . $x . '</option>';
						}
			$output .=	'</select> <small>' . __('(# of slides visible at the same time)', 'themify') . '</small>
					</p>
					<p>
			...
			$output .= '</select>
					...
					<p>
						...
						foreach ( $wrap_options as $name => $value ) {
							$output .= '<option value="' . $value . '" ' . selected( themify_get( 'setting-footer_slider_wrap' ), $value, false ) . '>' . $name . '</option>';
						}
			$output .=	'</select>
					</p>
					<hr />
