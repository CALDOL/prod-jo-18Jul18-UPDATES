...
/** ADD EDIT USERS TO ADMIN BAR */
add_action('admin_bar_menu', 'jo_add_toolbar_items', 100);
function jo_add_toolbar_items($admin_bar)
{
   if(current_user_can('manage_options')) {
        ...
        'title' => 'Approve Users',
        'parent' => 'site-name',
        'href' => get_site_url() . '/wp-admin/users.php?page=new-user-approve-admin',
        'meta' => array(
            'title' => __('Approve Users'),
        ),
    ));
}
}

function jo_cc_pl_redirect_notice ($content){

    $targetMessage = '';

    if (is_page('cc-pl-on-jo')) {

        //echo "<h1>Redirect worked</h1>";
        $isRedirect = get_query_var('tl', -1);
        //var_dump($isRedirect);

        switch ($isRedirect) {

            case 'cc':
                $targetForum = "Company Command";
                $targetImage = get_site_url() . "/wp-content/uploads/2018/05/cc-army-mil-splash.png";
                break;

            case 'pl':
                $targetForum = "Platoon Leader";
                $targetImage = get_site_url() . "/wp-content/uploads/2018/05/pl-army-mil-splash.png";
                break;

            case 'ms':
                $targetForum = "MilSpace";
                $targetImage = get_site_url() . "/wp-content/uploads/2018/05/FinalMilSpaceScreenShot.png";
                break;
        ...
        $targetMessage = "<h1>Thank you for your interest in the " . $targetForum . " Forum!</h1><p><img style='float:left; width:50%; margin:0 30px 55px;' src='" .  $targetImage . "'/> </p>";
        $targetMessage .= "<p>The " . $targetForum . " Forum provided many years of ";
        $targetMessage .= "great content and even better conversation and collaboration.  The " . $targetForum . " Forum has now evolved into the Junior Officer (JO) Forum.</p><p>Please consider registering for the JO Forum by clicking on the Register button below.</p>";
    }
    return $targetMessage . $content;

}
add_action('the_content', 'jo_cc_pl_redirect_notice');

function jo_cc_pl_redirect_notice_ext ($content){
...
add_action('template_redirect', 'jo_cc_pl_redirect_notice_ext');
...
function redirect_to_dropbox() {
  if(!is_admin() && is_single('rndv2018')) {
    wp_redirect("https://www.dropbox.com/sh/y0gqj0pmqrdul4g/AADIf0gYHDarb1-gWNvquM7xa?dl=0");
    exit();
  }
}
add_action('template_redirect', 'redirect_to_dropbox');
...
function jo_read2lead_book_data_shortcode($atts=[], $content=null){

    //var_dump($atts['isbn']);

	$url = "https://www.googleapis.com/books/v1/volumes?q=" . $atts['isbn']  ;
	$json = file_get_contents($url);
	$json_data = json_decode($json, true);
	echo "<img style='float: left; margin-right: 15px;' src='" . $json_data['items'][0]['volumeInfo']['imageLinks']['thumbnail'] . "''>";
	echo "<h1 class='r2l-title'>". $json_data['items'][0]['volumeInfo']['title'] . "<br/>";
	echo "<span class='r2l-subtitle'>" . $json_data['items'][0]['volumeInfo']['subtitle'] . "</span></h1>";
	echo "<h4 class='r2l-meta-data'>By " . $json_data['items'][0]['volumeInfo']['authors'][0] . "&nbsp;&nbsp;&nbsp;Published ". $json_data['items'][0]['volumeInfo']['publishedDate'] . ", " . $json_data['items'][0]['volumeInfo']['pageCount']  . " pages</h4>";
  echo "<h4 style='margin-bottom: 20px;' class='r2l-description'>" . $json_data['items'][0]['volumeInfo']['description']  . "</h4>";
...
function jo_track_user_login_history($id,$user) {
	$tracking = get_user_meta($user->ID,'_user_login_history',true);
	if($tracking == ""){
...
function caldol_show_login_history($atts, $content=""){
    ...
    if($userid != -1) {
        ...
        $hasloggedin = get_user_meta($userid, 'pw_new_user_approve_has_signed_in', true);
...
function jo_title_with_views() {


	if ( ( is_page() && is_main_query() ) && get_current_user_id() == 2) {
		echo " (" . getPostViews(get_the_ID()) . ")";
	}

}
add_action( 'themify_content_start', 'jo_title_with_views' );
 ...
 $DateTime->modify('-4 hours');
    ...
    if($DateTime->format('w') == 2){
	  ...
	  //$returnString .= '<span id="tools-open-help"><img src="' . get_stylesheet_directory_uri() . '/images/question_blue.png" alt="" width="" height="" /></span>';
	    $returnString .=  "<div id='tuesday-tools-help' style='display: block;'><img id='tuesday-tools-pic' src='/wp-content/uploads/2017/07/soldier-coffee-today-is-tuesday-tools.jpg'/>Every
...
<div id="maintenance-warning" style="display: none; border: 4px solid maroon; width: 75%; margin: 0 auto; text-align: center; background-color: cornsilk;" >
  
  <h3 style="display: none; font-weight: bold;color: red;">The Junior Officer Forum will be down for maintenance from<br/>0800-1200, Wednesday, 14 March 2018</h3>
  <h3 style="display: none;">If you need to contact the JO Forum, we can be reached via email at <a href="mailto:jo.team@usma.edu">JO.Team@usma.edu</a></h3>
  
  <h3 style="display: block; font-weight: bold;">The JO Forum updates are complete!<br/>If you find any bugs or gremlins, please let us know by reporting it as a bug in the 
  <a href="https://juniorofficer.army.mil/discussion/bughunter/">Bughunter</a> discussion thread or use the 
	<a href="contact-us/">Contact Us form</a>.</h3>
...
echo "<p class='gmail-warning-header'>IMPORTANT NOTICE ABOUT EMAILS FROM THE JO FORUM <span id='show-gmail-warning-text'><a href='#'>Read Notice</a></span></p>";
...
echo "</div>";
  ...
  echo "<div id='no-gmail'>";
  echo "<p><span style='font-weight: bold; font-size: 1.2em;'>ANNOUNCEMENT:  No more email from GMail!</span><br/>Our JO Forum server is now authorized to send emails to .mil addresses.<br/>";
  echo "<span style='font-weight: bold; font-size: 1.2em;'>The new FROM email address is <span style='color: blue;'>no-reply@juniorofficer.army.mil</span></emphasis></span><br/>Please ensure you allow that address within any \"junk email\" rules you may have set.</p>";
  echo "</div>";
...
function caldol_modify_tab_forum_pagination_links($pageLinks){

	global $post;
	$post_slug=$post->post_name;
    //var_dump($pageLinks);
  $checkTarget = get_the_permalink()."/";

	if(is_home() || is_front_page() || slugContains($checkTarget, ['forum/junior-officer'])){

        return $pageLinks;
	}
	else {

		if ( slugContains( $post_slug, ['-community', '-toc'] ) ) {
			$pageLinks = str_replace("href='/'", "href='/".$post_slug."'", $pageLinks);
			$modifiedTabPageLinks = str_replace( "/page/",
				"/" . $post_slug . "/page/",
				$pageLinks );


			return $modifiedTabPageLinks;
            }
	}
...
function slugContains($haystack, $needles){
...
function endsWith($haystack, $needles)
{
	foreach((array) $needles as $needle) {
		...
		$isValid = substr( $haystack, - $length ) === $needle;
...
add_filter( 'bbp_get_forum_pagination_links', 'caldol_modify_tab_forum_pagination_links' );
...
function caldol_register_custom_views() {
	...
	bbp_register_view( 'all-forums', __( 'All Forums' ),
...
preg_match( '/\.mil@mail\.mil|\.fjo@mail\.mil|us\.army\.mil|navy\.mil|usmc\.mil|usafa\.edu|usna\.edu/', $EEmail, $matches );
...
function setPostPie($postID) {
	...
	if ( $count ) {
		...
		$currPostParent  = get_post( $currPost->post_parent);
		...
		$parentTitle =  $currPostParent->post_title;
		$parentLink =  get_permalink($currPostParent->ID);

		$typePost = "";
		$pieMessage = '';

		if($currPost->post_type == 'reply'){

		$currPostLink = $parentLink . '#post-' . $currPost->ID;

		    $pieMessage = "$authorFirstName,\r\n\r\n<br/>JO member <a href='$userLink'>$userUserName</a>, found your <a href='$currPostLink'>reply</a> to the topic \"<a href ='$parentLink'>$parentTitle</a>\" to be Positive, Inspiring, and Energizing.  \r\n\r\n<br/></br>Thank you for serving up PIE in the Junior Officer forum.  Contributors like you make a positive impact on our profession. \r\n\r\n<br/><br/>-- The Junior Officer Forum Team";
        }
        else{

		    $pieMessage = "$authorFirstName,\r\n\r\n<br/>JO member <a href='$userLink'>$userUserName</a>, found your contribution of \"<a href ='$currPostLink'>$currPostTitle</a>\" to be Positive, Inspiring, and Energizing.  \r\n\r\n<br/>Thank you for serving up PIE in the Junior Officer forum.  Contributors like you make a positive impact on our profession. \r\n\r\n<br/>-- The Junior Officer Forum Team";
        }
		...
		if ( wp_mail( $authorEmail, 'PIE from Junior Officer Forum', $pieMessage, $headers ) ) {
			//echo "mail sent";
		} else {
			echo "mail failed";
		}
		...
		/*
	 	if ( wp_mail( $authorEmail, 'PIE from Junior Officer Forum', "$authorFirstName,\r\n\r\nJO member $userUserName ($userLink), found your contribution of \"$currPostTitle\" ($currPostLink) (post type is: $typePost) to be Positive, Inspiring, and Energizing.  \r\n\r\nThank you for serving up PIE in the Junior Officer forum.  Contributors like you make a positive impact on our profession. \r\n\r\n-- The Junior Officer Forum Team", $headers ) ) {
			//echo "mail sent";
		} else {
			echo "mail failed";
		}
...
function test_caldol_like_replies($content, $reply_id) {
	...
	if(!$userPied){
		$content .=  '<button value="' . $reply_id . '" class="pieButton showPieButton pieButton_' . $reply_id . '" title="Did you find this Positive, Inspiring, or Energizing (PIE)?. If so, click the button and an email will be sent to the author to let them know you thought it was worthy of some PIE!">Send PIE</button>';
...
//add_action('admin_init', 'caldol_confirm_comment_edit');
function caldol_confirm_comment_edit($currentUserID){

    //$currentUserID = get_current_user_id();

    $currentAction = isset($_GET['action'])?$_GET['action']:-1;


    if($currentAction == 'editcomment' && current_user_can('edit_posts')   ){

        $targetCommentID = isset($_GET['c'])?$_GET['c']:-1;


        $commentAuthorID = get_comment($targetCommentID)->user_id;
        if($currentUserID == $commentAuthorID) {
            ...
            //remove_menu_page('edit.php?post_type=leadercast');
        ...
        //die('line 2675 - user can manage_options');
        // return;
    }
...
/* CHECK IF THIS IS A REQUEST TO EDIT A COMMENT */
    if(isset($_GET['c']) && caldol_confirm_comment_edit($currentUserID)){
        return;
    }
	...
	if (  !current_user_can( 'edit_others_pages' ) && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
		...
		exit;
            ...
            if ($targetPage != -1 && array_key_exists($targetPage, $leadList)) {
                ...
                //CALDOL CHANGE
                //die($targetPage . 'line 2710, functions.php');
...
//add_action( 'admin_init', 'caldol_redirect_non_admin_users' );
function VERSION2_caldol_redirect_non_admin_users() {
...
$currentUserID = get_current_user_id();
    ...
    $leadList = array(array());
    $leadList[50] = array('cc-toc', 90); 
    $leadList[367] = array('cc-community', 90);
    
    $leadList[60] = array('staff-toc', 184); 
    $leadList[373] = array('staff-community', 184);
    
    $leadList[61] = array('xo-toc', 39); 
    $leadList[377] = array('xo-community', 39); 
    $leadList[62] = array('pl-toc', 1); 
    $leadList[370] = array('pl-community', 1); 
    $leadList[63] = array('bolc-toc', 784); 
    $leadList[342] = array('bolc-community', 784); 
    $leadList[948] = array('arng-usar-toc', 163); 
    $leadList[935] = array('arng-usar-community', 163); 
    $leadList[2129] = array('read2lead', 1771, 6); 
    $leadList[2115] = array('read2lead', 1771, 6); 

    $leadList[395] = array('leadership-topic', 2031); 
    $leadList[354] = array('soldiers-and-families-topic', 1); 
    $leadList[433] = array('training-topic', 1); 
    $leadList[702] = array('planning-topic', 682); 
    $leadList[390] = array('fitness-topic', 149); 
    $leadList[421] = array('self-development-topic', 1);
    
    $leadList[383] = array('additional-duties-topic', 1); 
    $leadList[400] = array('maintenance-topic', 1); 
    $leadList[405] = array('operations-topic', 1); 
    $leadList[410] = array('personnel-topic', 1); 
    $leadList[426] = array('supply-topic', 2269); 
	...
	if( !is_user_logged_in() &&
	      ...
	      ! is_home() &&
			    ...
			    (!is_feed("leadershuddle") && !is_category('leadershuddle')) &&
	       		!is_page( "latest-leaders-huddle-episode" ) &&
                !is_page("army-magazine") &&
           		!is_page("cc-pl-on-jo")
                )			 
